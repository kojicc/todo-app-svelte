<script>
	...
	$effect(() => {
		if (browser && userInfo?.username && isInitialized) {
			const userName = userInfo.username;
			const savedTodos = localStorage.getItem(`svelte-todos-${userName}`);
			const savedNextId = localStorage.getItem(`svelte-nextId-${userName}`);
	...
	$effect(() => {
		if (browser && isInitialized && userInfo?.username) {
			const userName = userInfo.username;
...
<div>
	<Header {isLoggedIn} {userInfo} onLogout={logout} {incompleteCount} {completedCount} />
